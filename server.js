...
const findDuplicates = (arr) => {
  ...
  return results;
	...
	console.log('Socket connected. ID: ' + socket.id);
  	...
  	var toExactMinute = 20000 - (new Date().getTime() % 20000);
	var	song_name='';
	var song_artist='';
	var song_uri='';
	var song_image='';  
											...
											if(song_name!=response.data['item']['name'] && song_artist!=response.data['item']['album']['artists'][0]['name']){
												...
												socket.emit('event', song_name+' - '+song_artist);
												 ...
												 var records = [
												  ...
												  db.query("INSERT INTO user_song_preview (user_id, song_uri,song_image,song_name,song_artist,proccess_time) VALUES ?", [records], function (err, result, fields) {
...
/*
				...
				if(err) throw err;
					rowse.forEach(response_row=>{
							 ...
							 if(matches!=response_row['song_name'] ){
								matches= response_row['song_name'];
